...
const levelers = [
    ...
    32, 33, 35, 36, 38, 40, 42, 45, 48, 51,
    55, 60, 
];
const levelersDouble = [
    1, 2, 3
];
class Skill {
    ...
    maintain() {
        if (this.level < c.SKILL_CAP) {
            if (this.score - this.deduction >= this.levelScore) {
                ...
                this.level += 1;
                this.points += this.levelPoints + this.levelPointsDouble;
  ...
  get levelPointsDouble() {
        if (levelersDouble.findIndex(e => { return e === this.level; }) != -1) { return 2; } return 0;
    ...
    constructor(body, info) {
      this.color=16;
        ...
        this.color=info.PROPERTIES.COLOR
            ...
            this.shootOnShift = (info.PROPERTIES.SHOOT_ON_SHIFT == null) ?
                false : info.PROPERTIES.SHOOT_ON_SHIFT;
        }                    
...
class Entity {
    constructor(position, master = this) {
        ...
        this.shiftamount_x = 0;
        this.shiftamount_y = 0;
    ...
    define(set) {
      ...
      if (set.TANKSHIELD != null) { 
            this.tankshield = set.TANKSHIELD; 
        }  
      ...
      if (set.FREEZESPEED != null) {
        ...
        if (set.FROZEN_STATUS != null) {
          this.frozenStatusEffect = set.FROZEN_STATUS
        }
        if (set.FREEZEIMMUNE != null) {
          this.freezeImmune = set.FREEZEIMMUNE
        }
        if (set.NECRO != null) { 
        ...
        if (set.FOVTOGGLE != null) { 
            this.FOVtoggle = set.FOVTOGGLE; 
        }else {
          this.FOVtoggle = null;
        }
        if (set.DANGER != null) { 
    ...
    refreshBodyAttributes() {
        ...
        this.shield.set(
            (((this.settings.healthWithLevel) ? 0.2 * this.skill.level : 0) + this.SHIELD) * (this.skill.shi/3), 
      ...
      if (this.shootOnDeath) {
        ...
        if (this.shield.max) {
            if (this.damageRecieved !== 0) {
                ...
                if(shieldDamage > 0.1){this.hit = 1}
            }
        }
        // Health damage 
        if (this.damageRecieved !== 0) {
            ...
            if(healthDamage > 0.1){this.hit = 1}
        }
        this.damageRecieved = 0;
        // Check for death
        if (this.isDead()) {
      ...
      if (gun.shootOnDeath) {
        ...
        gun.fire(gx, gy, sk);
            ...
            this.collisionArray.forEach(instance => {
                ...
                if (instance.type === 'puller') return 0;
...
const sockets = (() => {
  ...
  function MakeAnimations(trueLabel, totalFrames, definition, speed, switcheroo){
  ...
  classChange()
            ...
            if (player.body.labelswitch == trueLabel + '0')  {
                        animation = true
                for (let i=0; i<=totalFrames; i++){
        setTimeout(() => {
        player.body.define(Class[definition + (i + (i==0))]);
        }, speed * i);}  
          player.body.define(Class[definition + totalFrames])         
                      }else{
            if (switcheroo != true){
              if (player.body.labelswitch == trueLabel + totalFrames)  {
                animation = true
              for (let i=0; i<=totalFrames; i++){
        setTimeout(() => {
        player.body.define(Class[definition + (totalFrames - (i + (i==0)))]);
        }, speed * i);}
          player.body.define(Class[definition + '0'])}
              }
          }
                    };
            
//player.body.shiftamount_y = 99
//player.body.shiftamount_x = 99
            ...
            //MakeAnimations(the base labelSwitch of the tanks, the number of total frames, the base label for the tanks, speed with 1 being fast and 100 being normal)
                MakeAnimations('Ac', 15, 'akafugi', 100)
                MakeAnimations('Man', 15, 'manufac', 100)
            ...
            //switcheroo code stuff that you don't care about unless you're making chain animations:
              ...
              switch (player.body.labelswitch) {
    case 'SMa0':
      MakeAnimations('SMa', 15, 'switcherooMA', 50, true)
    break;
    case 'SBa0':
      MakeAnimations('SBa', 15, 'switcherooBA', 50, true)
    break;
    case 'STw0':
      MakeAnimations('STw', 15, 'switcherooTW', 50, true)
    break;
    case 'SSn0':
      MakeAnimations('SSn', 15, 'switcherooSN', 50, true)
    break;
    case 'SFl0':
      MakeAnimations('SFl', 15, 'switcherooFL', 50, true)
    break;
    case 'SDi0':
      MakeAnimations('SDi', 15, 'switcherooDI', 50, true)
    break;
    case 'SLa0':
      MakeAnimations('SLa', 15, 'switcherooLA', 50, true)
    break;
    case 'SAu0':
      MakeAnimations('SAu', 15, 'switcherooAU', 50, true)
    break;
  };
                    ...
                    if (player.body != null) {
                        if (player.body.id === e.master.id) {
                            ...
                            // Set the proper color if it's on our team
                             if (player.body.color == 12 && c.MODE == 'ffa') {
                             data[12] = 10//[10, 11, 12, 15][player.team - 1];
                         } 
                            ...
                            if (player.body != null) {
                                ...
                                // I live!
                                    ...
                                    // Update camera position and motion
                                                  if(player.body.shiftactive == false){
                          player.body.shiftamount_x = 0
                          player.body.shiftamount_y = 0
                                                      }
                                    camera.x = player.body.photo.x + player.body.shiftamount_x;
                                    camera.y = player.body.photo.y + player.body.shiftamount_y;  
                                    camera.vx = player.body.photo.vx;
                                    camera.vy = player.body.photo.vy;  
                          //player.body.shiftactive = false
                            ...
                            // Update my stuff
                          // still working on this but eventually UIA
                          // this below if done right will allow for FoV toggle
                          // Once I figure out how to match a mouse position to the FoV change I will replace the 0s, or you can change it for everything by changing them here
                ...
                if (goahead) {
                  ...
                  if (t) { // Only if we still need to find the collision
                    ...
                    if (doDamage) {
                      ...
                      if (n.poison) {
                        my.poisoned = true
                        my.poisonedLevel = n.poisonToApply
                        my.poisonTime = 20
                        my.poisonedBy = n.master
                      }
                      if (my.poison) {
                        n.poisoned = true
                        n.poisonedLevel = my.poisonToApply
                        n.poisonTime = 20
                    ...
                    /*************   FREEZE  ***********/
                      if (n.freeze) {
                        my.frozen = true
                        my.frozenStatus = n.frozenStatusEffect
                      ...
                      if (my.freeze) {
                        n.frozen = true
                        n.frozenStatus = my.frozenStatusEffect
		...
		if (element.poisoned) {// && element.type == 'tank'
           ...
           var poisondeath = 1
            if (element.health.amount <= 0){poisondeath = 0}
              //this is different just because poison damage is calculated in percentages not amounts, meaning it would be OP against high health bosses
                //this makes it so that it deals 3x less damage to minibosses
                if (!element.invuln && element.type == 'miniboss' && element.health.amount > element.health.max / (165 - element.poisonLevel)) {
              element.health.amount -= element.health.max / (165 - element.poisonLevel)
              element.shield.amount -= element.shield.max / (105 - element.poisonLevel)
              }
            if (!element.invuln && element.type != 'miniboss' && element.health.amount > element.health.max / (55 - element.poisonLevel)) {
    ...
    function freeze(element) {
      entities.forEach(function(element) {
		...
		if (element.frozen) {// && element.type == 'tank'
            if(random > 0.994 && element.freezeStatus == true){
            let x = element.size + 10
            let y = element.size + 10
            Math.random() < 0.5 ? x *= -1 : x
            Math.random() < 0.5 ? y *= -1 : y
            ...
            if (element.freezeImmune == true){element.frozen = false}else{
...
var maintainloop = (() => {
  ...
  function placeRoids() {
    ...
    // Spawning functions
       /* return census => {
            if (timer > 30 && ran.dice(100 - timer)) {
                ...
                setTimeout(boss.spawn, 10);
                // Set the timeout for the spawn functions
            } else if (!census.miniboss) timer++;
        };
    })();*/
  let spawnCrashers = census => {//code by Octo originally, I had my own design but this is more efficient
    if (ran.chance(1.4 -  0.5 * census.crasher / room.maxFood / room.nestFoodAmount)) {
            let spot, i = 30;
            do { spot = room.randomType('nest'); i--; if (!i) return 0; } while (dirtyCheck(spot, 100));
var itemCrasher = {//this is the different part about their code, they use a dictionary to assign each name a point value which affects how often it's called
  'crasher':80,
  'sentryGun':1,
  'sentrySwarm':1,
  'sentryTrap':1,
...
function get(input) {
    var crasherarray = []; // Just Checking...
    for(var itemCrasher in input) {
        if ( input.hasOwnProperty(itemCrasher) ) { // Safety
            for( var i=0; i<input[itemCrasher]; i++ ) {
                crasherarray.push(itemCrasher);
            }
        }
    }
    return crasherarray[Math.floor(Math.random() * crasherarray.length)];
}
            let type = Class[get(itemCrasher)];
  ...
  /*let spawnBosses = census => {
    //UIA- delete the /* around this and add exports to do boss spawn
    // so these functions pretty much just well... spawn stuff. The one below is for bosses and the ones above can spawn multiple items at different rates and speed.
        if (ran.chance(0.1) && c.MAX_BOSS > bosslist.length) { // rate of spawn
            let spot, i = 30;
            do { spot = room.randomType('nest'); i--; if (!i) return 0; } while (dirtyCheck(spot, 100));
          let bossToSpawn = []// put in whatever you have for bossed to spawn
          let bossNames = []//what you want the message to be: ex. 'n EK-I' -> A*n EK-I* has spawned!
          var randomNum = [Math.floor(Math.random() * bossToSpawn.length)]
          util.log(randomNum)
            let type = (bossToSpawn[randomNum]);
          let o = new Entity(spot);
                o.define(type);
                o.define({SIZE:30}); //make size whatever i guess
                o.team = -100; //you can spawn it on teams... not sure why you would want to but for non-rouge just do a number 1-4
                bosslist.push(o); // adds it to a list to keep track so you don't have more than the max boss list in config bosses at one time (set to 1 default)
                sockets.broadcast('A ' + bossNames[randomNum] + ' has spawned!');
        }
    //exact same thing except this allows for bosses to spawn in the norm zones occasionally
    if (ran.chance(0.01) && c.MAX_BOSS > bosslist.length) { // normally less likely to spawn than in nest 
            let spot, i = 30;
      ...
      bosslist = bosslist.filter(e => { return !e.isDead(); });
    };
*/
    // The NPC function
    let makenpcs = (() => {
        ...
        return () => {
            ...
            // Spawning
            spawnCrashers(census);
            //UIA- see other part of this to use      spawnBosses(census);
         
            var type = Math.random()
            //UIA- use this to add bots to your server. Yes, they lag it out a bit but good for when you have no friends
            if (type >= 0.2){//these are the gun bots, like overlords and destroyers
        var botlist = ['botGun', 'botGun2', 'botBigGun', 'botGlassBarrel'];//a couple are defaulted to you, look at the bottom of definitions and un-comment them. You can also make your own, this list is just the stats
            
        var tanklist = ['basic', 'twin'];//these are the actual tank used, such as basic and stuff. add whatever but depending on the class's stats it might go wrrrrr
            }
          if (type < 0.2){//these are spawned less but ram tanks, like spikes and boosters
        var botlist = ['botRammer',  'botHeavyRammer'];//these are the same but have better body stats
            
        var tanklist = ['booster', 'spike'];//just a default, add whatever
            }
             var j = [Math.floor(Math.random() * botlist.length)]
             var k = [Math.floor(Math.random() * tanklist.length)]// these just choose a random one from each list, no editing needed
                if (bots.length < c.BOTS) {
                    let o = new Entity(room.random());
                    o.color = 17;
                    o.define(Class[botlist[j]]);
                    o.define(Class[tanklist[k]]);
                ...
                // pretty much just instantly upgrades everything, this can be edited so they have to earn XP and this is reccommended if you are turning off level up cheat
                // to do this, its a long complicated process so listen carefully. the first step is to set the   o.skill.score +=   to 0
                // step 2 is that there is no step 2.
                bots.forEach(o => {
                    if (o.skill.level < c.TIER_4) {// if you only have 3 tiers make it 45
                        o.skill.score += 1000;
                        o.skill.maintain();
                    }
                });
