...
const room = {
  ...
  scale: {
    square: (c.WIDTH * c.HEIGHT) / 300000000 ,
    linear: Math.sqrt((c.WIDTH * c.HEIGHT) / 300000000 )
  },
  maxFood: ((c.WIDTH * c.HEIGHT) / 15000 ) * c.FOOD_AMOUNT,
  isInRoom: location => {
    return (
      location.x >= 0 &&
      location.x <= c.WIDTH &&
...
class io_avoid extends IO {
  ...
  think(input) {
    ...
    this.avoid = nearest(entities, { x: this.body.x, y: this.body.y }, function(
      test,
      sqrdst
    ) {
      return (
...
class Gun {
  ...
  bulletInit(o) {
    ...
    o.define({
      ...
      LABEL:
        this.master.label + (this.label ? " " + this.label : "") + " " + o.label
    });
